#!/bin/bash
#SBATCH --job-name=ucp_lru_4c
#SBATCH --mail-type=END,FAIL
#SBATCH --mail-user=shruthikrishnan@tamu.edu
#SBATCH --ntasks=1
#SBATCH --time=12:00:00
#SBATCH --output=ucp_lru_4c_%j.log
...
echo "Running 4c LRU Mix A (bwaves, mcf, gcc, omnetpp)"
bin/champsim_lru_4c \
  ...
  > results/lru_4c_mixA.txt

# Mix B: mixed sensitivity
echo "Running 4c LRU Mix B (bwaves, mcf, x264, leela)"
bin/champsim_lru_4c \
  --warmup-instructions 50000000 \
  --simulation-instructions 250000000 \
  $TRACES/603.bwaves_s-3699B.champsimtrace.xz \
  ...
  > results/lru_4c_mixB.txt

# Mix C: cache-insensitive heavy
echo "Running 4c LRU Mix C (x264, leela, deepsjeng, xalancbmk)"
bin/champsim_lru_4c \
  --warmup-instructions 50000000 \
  --simulation-instructions 250000000 \
  $TRACES/625.x264_s-18B.champsimtrace.xz \
  ...
  > results/lru_4c_mixC.txt

echo "Done: LRU 4-core runs"
